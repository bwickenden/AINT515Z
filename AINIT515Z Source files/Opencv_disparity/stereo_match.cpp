...
*/
    ...
    if( !intrinsic_filename.empty() )
    {
        ...
        while (inLOOP){
           ...
           // Rect target = Rect(X-32, Y-32, 32, 32);
            ...
            if( true )
            {
               ...
               //  imshow("disparity2", dispcopy);


                Rect target (X-120, Y-32, 32, 32); //Rect target (320+100, 240-32, 64, 64);
                Mat disp8copy;
                disp8.copyTo(disp8copy);
                rectangle( disp8copy,target, Scalar::all(255), 2, 8, 0 ); // draw white rect
                //imshow("Dispcopy",dispcopy);
                imshow("Disp",disp8copy);
                ...
                int counting = 0;
                              ...
                              // Find the intenisty of pixels over a given area
                              for (int j =(Y-208+10); j<=(Y-10); j++){ // for loop to increase y
                                for (int i = (X-420+10); i<=(X-10); i++ ){ // for loop to increase x
                                    intensity1 = disp8.at<uchar>(j, i);
                                    Value = Value + intensity1; // Add total of intensity value
                                    counting = counting + 1;
                                }
                              }
